# exonware/xwauth.connector/tests/1.unit/security_tests/test_mfa_passkeys_security.py
...
from exonware.xwauth.identity.authentication.challenge_store import WebAuthnChallengeStore

from exonware.xwauth.identity.authentication.attestation_trust import build_pem_root_certs_bytes_by_fmt

from exonware.xwauth.identity.errors import XWInvalidRequestError
from exonware.xwauth.identity.security.backup_codes import generate_backup_codes, hash_backup_code, verify_backup_code
from exonware.xwauth.identity.security.mfa_policy import attestation_for_profile, merge_amr_claims, require_backup_codes
from exonware.xwauth.identity.security.mfa_secrets import decrypt_totp_secret, encrypt_totp_secret

from exonware.xwauth.identity.authentication.webauthn_factory import create_webauthn_credential_index_redis
...
def test_totp_encrypt_round_trip() -> None:
    ...
    cfg = XWAuthConfig(jwt_secret="unit-test-secret-for-mfa-encryption-key-derivation", allow_mock_storage_fallback=True)
...
def test_config_webauthn_resident_key_validates() -> None:
    ...
    XWAuthConfig(jwt_secret="s", webauthn_resident_key="required", allow_mock_storage_fallback=True).validate()
...
async def test_resolve_user_for_webauthn_credential_scan_and_cache(auth) -> None:
    ...
    from exonware.xwauth.identity.authentication.webauthn_credential_index import (
...
def test_webauthn_manager_requires_challenge_handle() -> None:
    ...
    from exonware.xwauth.identity.authentication.webauthn import WebAuthnManager
    ...
    auth = XWAuth(
        config=XWAuthConfig(
            ...
            webauthn_allow_insecure_defaults=True,
            allow_mock_storage_fallback=True,
        )
    )
...
def test_create_credential_index_redis_factory() -> None:
    ...
    assert create_webauthn_credential_index_redis(XWAuthConfig(jwt_secret="s", allow_mock_storage_fallback=True)) is None
...
async def test_redis_backed_index_resolve_after_process_cache_clear(auth) -> None:
    ...
    from exonware.xwauth.identity.authentication import webauthn_credential_index as mod
