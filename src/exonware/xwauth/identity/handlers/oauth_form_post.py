# exonware/xwauth.identity/handlers/oauth_form_post.py
"""HTML document for OIDC/OAuth2 ``response_mode=form_post`` (auto-submit POST to client ``redirect_uri``).

Implementation module for OIDC ``form_post`` HTML. **Public façade:** ``exonware.xwauth.identity.form_post_connector``
(GUIDE_32). **xwauth** ``handlers.mixins.auth_core`` imports that façade when ``exonware-xwauth-identity``
