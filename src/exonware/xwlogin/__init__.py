...
and (with optional extra ``[handlers]``) ``exonware.xwlogin.handlers`` — use ``from exonware.xwlogin.handlers import mixins`` (lazy) or ``handlers.mixins`` / ``handlers.authenticators`` for routes and authenticator factories.
